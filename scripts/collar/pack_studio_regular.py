"""Pack the regular-fit t-shirt line art into builder fill parts.

Same construction as the slim, boxy, and oversized packs: closed fabric fills,
...
Does not wipe slim, boxy, or oversized assets — writes `(regular)` named files
alongside.

Usage:
    python scripts/collar/pack_studio_regular.py --probe
    python scripts/collar/pack_studio_regular.py
"""
...
ART_IN = SRC / "regular-tshirt-lineart-bold.png"
KEYED = SRC / "regular-tshirt-lineart-transparent.png"
...
PART_SEEDS: list[dict] = [
    {"category": "Body", "asset": "Body (regular)", "seeds": [[511, 781], [190, 410], [833, 411]]},
    ...
    {"category": "Inner back neck", "asset": "Inner back neck (regular)", "seeds": [[517, 351]]},
]
...
def remove_previous_regular() -> None:
    for category, asset in REGULAR_FILES:
...
def pack(probe_only: bool) -> None:
    ...
    proof = SRC / "regular-tshirt-closed-regions.png"
    ...
    source_svg = SRC / "regular-tshirt-lineart.svg"
    source_svg.write_text(wrap_source_svg("Ceriga regular t-shirt line art", ink), encoding="utf-8")
    ...
    Image.fromarray(colour_proof).save(SRC / "regular-tshirt-fill-proof.png")
    print("wrote regular-tshirt-fill-proof.png")

    remove_previous_regular()
    for part in named:
        if not part["ids"]:
            continue
        write_part(
            part["category"],
            part["asset"],
            wrap_fill_only(f"Regular t-shirt - {part['asset']}", part["mask"]),
        )
    ...
    write_part(
        "Outline",
        "Outline (regular)",
        wrap_ink("Regular t-shirt - Outline", construction),
    )
    if stitches.any():
        write_part(
            "Stitching",
            "Cover stitch (regular)",
            wrap_fill_only("Regular t-shirt - Cover stitch", stitches),
        )

    from pack_denim_shorts import rasterize_svg

    print("raster 2048 / 4096...")
    rasterize_svg(source_svg, 2048, ss=4).save(SRC / "regular-tshirt-vector-2048.png")
    rasterize_svg(source_svg, 4096, ss=2).save(SRC / "regular-tshirt-vector-4096.png")
    print("done")
