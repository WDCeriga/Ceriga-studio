"""Pack the oversized drop-shoulder t-shirt line art into builder fill parts.

Same construction as the slim V-neck and boxy ringer packs: closed fabric fills,
black construction on top, dashed cover-stitch separate. Front crew + back band
are one Neck colour. Inner back (through the hole) is a lighter body colour.

Does not wipe slim or boxy assets — writes `(oversized)` named files alongside.

Usage:
    python scripts/collar/pack_studio_oversized.py --probe
    python scripts/collar/pack_studio_oversized.py
"""
...
ART_IN = SRC / "oversized-tshirt-lineart-bold.png"
KEYED = SRC / "oversized-tshirt-lineart-transparent.png"

# Seeds from --probe on the keyed 1024x1536 drawing.
# Neck = every collar rib sliver (front + back band) as one colour.
PART_SEEDS: list[dict] = [
    {"category": "Body", "asset": "Body (oversized)", "seeds": [[511, 923]]},
    ...
    {"category": "Inner back neck", "asset": "Inner back neck (oversized)", "seeds": [[510, 341]]},
]
...
def remove_previous_oversized() -> None:
    for category, asset in OVERSIZED_FILES:
...
def pack(probe_only: bool) -> None:
    ...
    proof = SRC / "oversized-tshirt-closed-regions.png"
    ...
    source_svg = SRC / "oversized-tshirt-lineart.svg"
    source_svg.write_text(wrap_source_svg("Ceriga oversized t-shirt line art", ink), encoding="utf-8")
    ...
    Image.fromarray(colour_proof).save(SRC / "oversized-tshirt-fill-proof.png")
    print("wrote oversized-tshirt-fill-proof.png")

    remove_previous_oversized()
    for part in named:
        if not part["ids"]:
            continue
        write_part(
            part["category"],
            part["asset"],
            wrap_fill_only(f"Oversized t-shirt - {part['asset']}", part["mask"]),
        )
    ...
    write_part(
        "Outline",
        "Outline (oversized)",
        wrap_ink("Oversized t-shirt - Outline", construction),
    )
    if stitches.any():
        write_part(
            "Stitching",
            "Cover stitch (oversized)",
            wrap_fill_only("Oversized t-shirt - Cover stitch", stitches),
        )

    from pack_denim_shorts import rasterize_svg

    print("raster 2048 / 4096...")
    rasterize_svg(source_svg, 2048, ss=4).save(SRC / "oversized-tshirt-vector-2048.png")
    rasterize_svg(source_svg, 4096, ss=2).save(SRC / "oversized-tshirt-vector-4096.png")
    print("done")
