...
export function PackagingDesignerPreview({ design, onChange, selectedId, onSelect, limited, onBoundary, garmentColor, garmentType }: { design: PackagingDesign; onChange: (design: PackagingDesign) => void; selectedId: string | null; onSelect: (id: string | null) => void; limited: boolean; onBoundary: (limited: boolean) => void; garmentColor: string; garmentType?: string }) {
  ...
  return <div className={`pkg-preview pkg-theme-${background}`} data-packaging-preview onPointerDown={event => event.stopPropagation()} onWheel={event => event.stopPropagation()}>
    <div className="pkg-preview-toolbar">
      ...
      }}>{view === 'panel' ? focusName : view.charAt(0).toUpperCase() + view.slice(1)}</button>)}{template.closure === 'drawer' && <button type="button" aria-pressed={design.view === 'panel' && design.selectedPanel === 'interior-base'} onClick={() => {
        onChange({ ...design, view: 'panel', selectedPanel: 'interior-base' }); onSelect(null); onBoundary(false);
      }}>Interior Base</button>}</div> : <span className="pkg-preview-title">Standard bulk shipping carton</span>}
