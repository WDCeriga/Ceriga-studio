...
export function PackagingDesignerPreview({ design, onChange, selectedId, onSelect, limited, onBoundary, garmentColor }: { design: PackagingDesign; onChange: (design: PackagingDesign) => void; selectedId: string | null; onSelect: (id: string | null) => void; limited: boolean; onBoundary: (limited: boolean) => void; garmentColor: string }) {
  ...
  return <div className="pkg-preview" data-packaging-preview onPointerDown={event => event.stopPropagation()} onWheel={event => event.stopPropagation()}>
    <div className="pkg-preview-toolbar">
      {!isBulk && <div className="pkg-actions" role="group" aria-label="Packaging view">{views.map(view => <button type="button" key={view} aria-pressed={design.view === view} onClick={() => { onChange({ ...design, view, selectedPanel: view === 'front' || view === 'back' ? view : design.selectedPanel }); onSelect(null); }}>{view === 'panel' ? PANEL_NAMES[design.selectedPanel] : view}</button>)}</div>}
    ...
    <div className="pkg-preview-footer">{!isBulk && <><label className="pkg-check"><input type="checkbox" checked={guides} onChange={event => setGuides(event.target.checked)} />Print-safe guides</label>{packagingTemplate(design.templateId).transparency && <label className="pkg-check"><input type="checkbox" checked={contents} onChange={event => setContents(event.target.checked)} />Contents</label>}<span role="status" className={limited ? 'pkg-boundary' : ''}>{limited ? 'Print boundary reached' : PANEL_NAMES[design.selectedPanel]}</span></>}
